class Start {
	...
	startup() {
		...
		paper = SVG().addTo('#canvasdiv').size(windowWidth, windowHeight);

		const pgWidth = 400;
		const pgHeight = pgWidth / a4factor;

		paper.rect(pgWidth, pgHeight).move(500, 20).fill("lightgray");

		new PageSelector(this);

		const lettis = paper.text("Lettis").move(windowWidth - 200, 120).scale(2, 2);

		const lettigroup = paper.group();
		const wauzigroup = paper.group();
		...
		const wauzis = paper.text("Wauzis").move(windowWidth - 200, 160).scale(2, 2);
		...
		const drucki = paper.text("Drucki").move(windowWidth - 200, 20).scale(2, 2);
		const deleti = paper.text("Deleti").move(windowWidth - 200, 60);
	...
	clearPage() {
		console.log("removing all elements " + Letter.onPage.length);
		Letter.onPage.forEach(item => {
			console.log(item);
			item.remove();
		}
		);
